const { sequelize, Category, Service, User, ServiceProvider, Booking, Review, CommunityForum, ForumComment, Admin } = require('../models');
...
const seedDatabase = async () => {
    try {
        ...
        if (!adminExists) {
            await Admin.create({
                username: 'admin',
                password: hashedPassword // admin / password123
            });
        ...
        const spExists = await ServiceProvider.findOne({ where: { email: 'provider@example.com' } });
        ...
        if (!spExists) {
            ...
            const existingSPs = await ServiceProvider.findAll();
            createdProviders.push(...existingSPs);
        }
        ...
        for (const name of categoriesData) {
            const [cat] = await Category.findOrCreate({ where: { cat_name: name } });
            categories[name] = cat;
        }
