...
const EditServiceModal = ({ isOpen, onClose, onServiceUpdated, service }) => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.put(`/services/${formData.service_id}`, formData);
            onServiceUpdated();
            onClose();
        } catch (error) {
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
            <div className="relative p-8 border-0 w-full max-w-lg shadow-[0_20px_50px_rgba(0,0,0,0.3)] rounded-[40px] bg-white animate-in zoom-in-95 duration-300">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h3 className="text-2xl font-black text-slate-900">Edit Service</h3>
                        <p className="text-slate-500 text-sm font-medium">Update your service details</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            <div className="relative group">
                                ...
                                >
                                    {uploading ? (
                                        ...
                                        <span className="text-indigo-600 truncate max-w-full">Image Changed ✓</span>
                                    ) : (
                                        "Click to Upload"
                                    )}
                                </label>
                    ...
                    <div className="flex gap-4 pt-4">
                        ...
                        <button type="submit" disabled={uploading} className={`flex-[2] px-8 py-4 bg-indigo-600 text-white rounded-3xl font-black transition-all hover:bg-indigo-700 shadow-xl hover:shadow-indigo-200 active:scale-95 uppercase tracking-widest text-xs ${uploading ? 'opacity-50 cursor-not-allowed' : ''}`}>Update Service</button>
                    </div>
                </form>
            </div>
        </div>
